...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === 'join') {
    ...
    try {
      ...
      await interaction.reply(`Joined **${channel.name}** and persistent mode is ON.`);
    } catch (err) {
      console.error(err);
      scheduleReconnect(interaction.guild.id, 8000);
      await interaction.reply({ content: 'Failed to join now. Reconnect will retry automatically.', ephemeral: true });
    }
    return;
  }
