...
- Double-clique sur `run-bot.bat` à la racine du projet.
- Il créera `.env` depuis `.env.example` si besoin et lancera le bot.

## Windows monthly restart
- Double-clique sur `schedule-monthly-restart.bat` pour créer une tache planifiee mensuelle.
- La tache lancera `run-bot.bat` une fois par mois.
